#!/usr/bin/env python3
"""Pobierz Google Places ATRAKCJE dla etapów 1-7 Toskanii i dodaj do fact_json.attractions."""
...
ATTRACTION_TYPES = [
    ...
    "church", "monastery", "castle", "ruins", "winery", "vineyard",
    "park", "national_park", "garden", "cultural_center",
    "spa", "thermal_bath",
]
...
def search_nearby(lat, lon, types, radius=3000):
    payload = {
        "includedTypes": types,
        "maxResultCount": 10,
...
for stage_n, route_id, fact_id in STAGES:
    print(f"\n=== Etap {stage_n} (route {route_id}) ===")
    points = load_gpx(route_id)
    if not points:
        print(f"  BRAK GPX"); continue

    samples = sample_points(points, step_km=10)
    all_attr = {}

    for lat, lon, km in samples:
        places = search_nearby(lat, lon, ATTRACTION_TYPES, radius=3000)
        for p in places:
            pid = p.get("id")
            if pid in all_attr:
                continue
    ...
    for a in attr_list[:5]:
        ...
        print(f"    km {a['km']:5.1f} | {a['distance_to_track_m']:4}m | [{t}] {a['name'][:40]} ({a.get('rating','?')}⭐)")
    if len(attr_list) > 5:
        print(f"    ... i {len(attr_list)-5} więcej")

    # Dodaj do fact_json (zachowaj istniejące attractions z OSM)
    row = conn.execute(f"SELECT fact_json FROM qbot_planning_facts WHERE id={fact_id}").fetchone()
    if row:
        fj = row['fact_json'] or {}
        # Połącz Google attractions z istniejącymi OSM attractions
        existing = fj.get('attractions', [])
        existing_names = {a.get('name','').lower() for a in existing}
        new_attrs = [a for a in attr_list if a['name'].lower() not in existing_names]
        fj['attractions_google'] = attr_list  # osobne pole żeby nie nadpisać OSM
        ...
        print(f"  DB updated: {len(attr_list)} Google atrakcji + {len(existing)} OSM")
