...
- Prośba o ATRAKCJE na trasie ("włącz/pokaż atrakcje dla trasy X", "wyłącz atrakcje dla trasy X", "dodaj atrakcje/zabytki do trasy") → route_attractions (WYMAGA route_id; enable=true włącza, enable=false wyłącza). Atrakcje = turystyczne, zabytki, muzea, galerie (kościoły jako zabytki), wyłącznie z Google Places w promieniu 1,5 km od trasy. TRWAŁY przełącznik per-trasa: domyślnie WYŁĄCZONE (wtedy atrakcje nie są w ogóle pobierane ani pokazywane). Narzędzie od razu odświeża POI, więc zmiana jest widoczna natychmiast. NIE używaj do sklepów/wody/godzin — od tego jest route_recompute scope='poi'.
