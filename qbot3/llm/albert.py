...
- Prośba o ATRAKCJE na trasie ("włącz/pokaż atrakcje dla trasy X", "wyłącz atrakcje dla trasy X", "dodaj atrakcje/zabytki do trasy") → route_attractions (WYMAGA route_id; enable=true włącza, enable=false wyłącza). Kanoniczne źródła to Wikipedia, Wikidata i OSM w korytarzu ok. 2 km; Google jest tylko pomocniczym sygnałem rankingu. Włączenie przelicza WYŁĄCZNIE osobną warstwę atrakcji, wyłączenie tylko ją ukrywa — żadna z tych operacji nie odświeża sklepów, wody ani jedzenia. Do logistyki użyj route_recompute scope='poi'.
