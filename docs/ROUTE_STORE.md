...
- `route_attractions` (write) — włącza/wyłącza osobną kanoniczną warstwę atrakcji `route_attractions_v2.2`. Wikipedia/Wikidane są pełną bazą semantyczną, selektywny OSM uzupełnia historię, dziedzictwo, fortyfikacje i wyjątkowe konstrukcje, a Google wspiera lokalizację i ranking. Każdy niepusty wynik jakościowy może zostać opublikowany; 12 kandydatów/100 km to cel i górny limit, nie minimum publikacji. Niepełny dodatek OSM ma jawny `DEGRADED_OSM`/`missing_chunks` i jest ponawiany z cache, ale nie ukrywa kompletnej bazy Wikipedii. Korytarz wynosi 2050 m, a odległość jest miękką karą. Operacja nie przelicza `route_poi_layer`, więc nie rusza sklepów, jedzenia ani wody. Analiza Trasy i Planer czytają ten sam opublikowany ranking.
