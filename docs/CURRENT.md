...
- **Kaseta per jazda (2026-08-10 cd.)**: `/ride-readiness` przyjmuje `cassette_override` + `cassette_cogs` z QExt2 (zapis do `qbot_v2.qext2_cassette_report`, dedup 10 min); QExt2 dosyła stan przełącznika w URL fetcha (commit `52cc470` na gałęzi `fix/kontrast-2026-08` repo QExt2 — wymaga builda i instalacji na Karoo). Definicje kaset w `qbot_v2.gear_cassette` (10-46 SRAM XPLR, 10-52 Garbaruk: 10,11,12,14,16,18,21,24,28,32,37,44,52), przypisania per jazda w `qbot_v2.ride_cassette` (48 jazd, heurystyka: przed ~25.05=10-46; AXS-config-52=10-52; >250 km od Wwy=10-52; reszta=10-46). Dwie niejednoznaczne: 20.06 Wyszogród i 04.07 Tarczyn (lokalne, ale config 52) — do potwierdzenia przez użytkownika.

- **[PODJAZDY-SKALA] faza 2 (2026-08-10)**: scorer −2…+2 działa w raporcie tras. Moduł `qbot3/routes/climb_score.py`: krzywa mocy osiągalnej = kwantyle %CP (P75/P90) z `ride_climb_efforts` (tylko na_limicie, bez kwarantanny; cache 15 min, fallback zaszyty), fizyka per 100 m segment (Crr wg kategorii nawierzchni `sk`), godne minimum tempa 6 km/h, crux = najstromsze ~200 m vs P90@2min, rozstrzyganie −1/−2 budżetem deficytu W′ (0.8·W′). Wpięte w `_build_report_data` (badge + „dlaczego" w zakładce Przewyższenia; kaseta zakładana heurystyką odległości od Wwy, masa = waga + 14 kg rower/osprzęt). Walidacja: trasa 9.08 — 10.7%×1.3 km = „0 podjadę" (95% osiągalnego), reszta +1/+2, zgodnie z realnym przebiegiem. OGRANICZENIA v1 (do TODO): brak temperatury w krzywej, brak łańcuchowania zmęczenia między podjazdami (każdy oceniany „na świeżo"), tryb DZIEŃ planera bez oceny, readiness dnia nieuwzględniany.

- **[PODJAZDY-SKALA] łańcuchowanie (2026-08-10, cd.)**: `chain_verdict` w `climb_score.py` — symulacja W′ przez CAŁĄ sekwencję podjazdów (segment po segmencie: strome rampy kosztują W′ nawet przy średniej < CP; odbudowa tau na lżejszych fragmentach i na dojazdach między podjazdami wg prędkości netto trasy). Polityka zachłanna: atak dopóki min W′ ≥ 12%, inaczej tempo (88% osiągalnego). Werdykt + piguły per podjazd (tryb, W′ wejście→min) w boxie na górze zakładki Przewyższenia; chip atak/tempo na kartach. Walidacja: „ciągle stromo" = atak #1–5, od #6 tempem (zgodne z realnym W′=0 na #6 dnia 9.08); „nowa 1" = atak wszystkich, min W′ 20%.
