...
from qbot_roadmap_runner import (
    ...
    _tool_qbot_roadmap_runner_reconcile_state,
...
from qbot_route_tools import (
    ...
    _tool_qbot_route_plan_analysis,
    _tool_qbot_ride_analysis,
...
from qbot_wellness_store import (
    _tool_qbot_wellness_db_status,
    _tool_qbot_garmin_wellness_import_preview,
    _tool_qbot_garmin_wellness_import_execute,
    _tool_qbot_garmin_sleep_import_preview,
    _tool_qbot_garmin_sleep_import_execute,
    _tool_qbot_intervals_wellness_import_preview,
    _tool_qbot_intervals_wellness_import_execute,
    _tool_qbot_intervals_comments_import_preview,
    _tool_qbot_intervals_comments_import_execute,
    _tool_qbot_cronometer_nutrition_import_preview,
    _tool_qbot_cronometer_nutrition_import_execute,
    _tool_qbot_wellness_day_get,
    _tool_qbot_sleep_day_get,
    _tool_qbot_nutrition_day_get,
    _tool_qbot_wellness_range_summary,
    _tool_qbot_nutrition_range_summary,
...
TOOLS_META: dict[str, dict[str, Any]] = {
    ...
    "qbot_roadmap_runner_resume": {
        "description": "Resume P0 runner after pause/block/fail",
        "category": "roadmap",
        "safe": True,
        "args_schema": {},
    },
    "qbot_roadmap_runner_reconcile_state": {
        "description": "Reconcile runner state with git commit log — discovers completed tasks",
        "category": "roadmap",
        "safe": True,
        "args_schema": {"block": "P0"},
    },
    ...
    "qbot_rwgps_artifact_store_status": {
        "description": "Sprawdza schemat i zapis artefaktów RWGPS w PostgreSQL",
        "category": "legacy",
        "safe": True,
        "args_schema": {},
    },
    ...
    "qbot_rwgps_route_export_file": {
        "description": "Wyeksportuj trasę RWGPS do lokalnego artefaktu GPX/TCX/JSON",
        "category": "legacy",
        "safe": True,
        "args_schema": {"route_id": "55291905", "format": "gpx", "return_mode": "metadata"},
    },
    "qbot_gpx_artifact_parse": {
        "description": "Przeanalizuj zapisany artefakt GPX i zwróć metadane śladu",
        "category": "legacy",
        "safe": True,
        "args_schema": {"artifact_path": "/opt/qbot/artifacts/exports/rwgps/rwgps_55256628.gpx", "return_mode": "summary"},
    },
    "qbot_route_plan_analysis": {
        "description": "PELNA ANALIZA TRASY / analiza planowanej trasy / oceń trasę — JEDYNE wlasciwe narzedzie do analizy zaplanowanej trasy. Zwraca gotowy, znormalizowany briefing: dystans, podjazdy, nawierzchnia (pudelka 80 m), wiatr per km, forma. Pogoda LICZONA PO WSPOLRZEDNYCH (nie po nazwie miasta). Domyslnie najnowsza otrasowana trasa. Args opcjonalne: artifact_id, route_id, start YYYY-MM-DD HH:MM, speed_kmh.",
        "category": "read",
        "safe": True,
        "args_schema": {"artifact_id": "", "route_id": "", "start": "", "speed_kmh": 22},
    },
    "qbot_ride_analysis": {
        "description": "FAZA B: ocena wykonanej jazdy — naloz FIT na pudelka planu + werdykt wobec formy. Domyslnie najnowszy FIT. Args opcjonalne: fit (sciezka), ride (ride_key).",
        "category": "read",
        "safe": True,
        "args_schema": {"fit": "", "ride": "latest"},
    },
    "qbot_route_artifact_enrich": {
        "description": "Wzbogacaj artefakt trasy o summary i opcjonalny profil nawierzchni",
        "category": "legacy",
        "safe": True,
        "args_schema": {
    ...
    "qbot_last_ride_location_status": {
        "description": "Znajduje ostatnią lokalizację z przejechanej trasy — read-only",
        "category": "legacy",
        "safe": True,
        "args_schema": {},
    },
    "qbot_resolve_weather_location": {
        "description": "Ustala lokalizację dla pogody — tekst wiadomości lub ostatnia trasa ≤18h",
        "category": "legacy",
        "safe": True,
        "args_schema": {"text": "jaka dziś jest pogoda?", "max_last_ride_age_hours": 18},
    },
    "qbot_weather_current": {
    ...
    "qbot_wellness_db_status": {
        "description": "Status lokalnej bazy wellness/sleep/nutrition — tabele, liczniki",
        "category": "wellness",
        "safe": True,
        "args_schema": {},
    },
    "qbot_garmin_wellness_import_preview": {
        "description": "Podgląd importu Garmin wellness do DB — sleep, HRV, body battery, RHR",
        "category": "wellness",
        "safe": True,
        "args_schema": {"date_from": "2026-05-01", "date_to": "today", "dry_run": True},
    },
    "qbot_garmin_wellness_import_execute": {
        "description": "Import Garmin wellness do PostgreSQL — TYLKO lokalny zapis",
        "category": "wellness",
        "safe": True,
        "args_schema": {"date_from": "2026-05-01", "date_to": "today", "dry_run": False},
    },
    "qbot_garmin_sleep_import_preview": {
        "description": "Podgląd importu Garmin sleep detail do DB",
        "category": "wellness",
        "safe": True,
        "args_schema": {"date_from": "2026-05-01", "date_to": "today", "dry_run": True},
    },
    "qbot_garmin_sleep_import_execute": {
        "description": "Import Garmin sleep detail do PostgreSQL",
        "category": "wellness",
        "safe": True,
        "args_schema": {"date_from": "2026-05-01", "date_to": "today", "dry_run": False},
    },
    "qbot_intervals_wellness_import_preview": {
        "description": "Podgląd importu Intervals wellness (HRV, waga, RHR) do DB",
        "category": "wellness",
        "safe": True,
        "args_schema": {"date_from": "2026-05-01", "date_to": "today", "dry_run": True},
    },
    "qbot_intervals_wellness_import_execute": {
        "description": "Import Intervals wellness do PostgreSQL",
        "category": "wellness",
        "safe": True,
        "args_schema": {"date_from": "2026-05-01", "date_to": "today", "dry_run": False},
    },
    "qbot_intervals_comments_import_preview": {
        "description": "Podgląd importu komentarzy/notes Intervals z wykrywaniem kalorii/makro z MFP",
        "category": "wellness",
        "safe": True,
        "args_schema": {"date_from": "2026-05-01", "date_to": "today", "dry_run": True},
    },
    "qbot_intervals_comments_import_execute": {
        "description": "Import komentarzy Intervals + parsowanie nutrition do PostgreSQL",
        "category": "wellness",
        "safe": True,
        "args_schema": {"date_from": "2026-05-01", "date_to": "today", "dry_run": False},
    },
    "qbot_cronometer_nutrition_import_preview": {
        "description": "Podgląd importu Cronometer nutrition do DB",
        "category": "wellness",
        "safe": True,
        "args_schema": {"date_from": "2026-05-01", "date_to": "today", "dry_run": True},
    },
    "qbot_cronometer_nutrition_import_execute": {
        "description": "Import Cronometer nutrition do PostgreSQL",
        "category": "wellness",
        "safe": True,
        "args_schema": {"date_from": "2026-05-01", "date_to": "today", "dry_run": False},
    },
    "qbot_wellness_day_get": {
        "description": "Pobiera dane wellness dla konkretnej daty z lokalnej DB",
        "category": "wellness",
        "safe": True,
        "args_schema": {"date": "today"},
    },
    "qbot_sleep_day_get": {
        "description": "Pobiera dane snu dla konkretnej daty z lokalnej DB",
        "category": "wellness",
        "safe": True,
        "args_schema": {"date": "today"},
    },
    "qbot_nutrition_day_get": {
        "description": "Pobiera dane żywieniowe dla konkretnej daty z lokalnej DB",
        "category": "wellness",
        "safe": True,
        "args_schema": {"date": "2026-05-03"},
    },
    "qbot_wellness_range_summary": {
        "description": "Podsumowanie wellness dla zakresu dat — średnie HRV, RHR, sen",
        "category": "wellness",
        "safe": True,
        "args_schema": {"date_from": "2026-05-01", "date_to": "2026-05-10"},
    },
    "qbot_nutrition_range_summary": {
        "description": "Podsumowanie żywieniowe dla zakresu dat",
        "category": "wellness",
        "safe": True,
        "args_schema": {"date_from": "2026-05-01", "date_to": "2026-05-10"},
    },
    "qbot_nutrition_db_status": {
        "description": "Status danych żywieniowych w DB — źródła, zakres dat",
        "category": "wellness",
        "safe": True,
        "args_schema": {},
    },
...
TOOLS: dict[str, Any] = {
    ...
    "qbot_roadmap_runner_reconcile_state": _tool_qbot_roadmap_runner_reconcile_state,
    ...
    "qbot_last_ride_location_status": _tool_qbot_last_ride_location_status,
    "qbot_resolve_weather_location": _tool_qbot_resolve_weather_location,
    ...
    "qbot_route_plan_analysis": _tool_qbot_route_plan_analysis,
    "qbot_ride_analysis": _tool_qbot_ride_analysis,
    ...
    "qbot_wellness_db_status": _tool_qbot_wellness_db_status,
    "qbot_garmin_wellness_import_preview": _tool_qbot_garmin_wellness_import_preview,
    "qbot_garmin_wellness_import_execute": _tool_qbot_garmin_wellness_import_execute,
    "qbot_garmin_sleep_import_preview": _tool_qbot_garmin_sleep_import_preview,
    "qbot_garmin_sleep_import_execute": _tool_qbot_garmin_sleep_import_execute,
    "qbot_intervals_wellness_import_preview": _tool_qbot_intervals_wellness_import_preview,
    "qbot_intervals_wellness_import_execute": _tool_qbot_intervals_wellness_import_execute,
    "qbot_intervals_comments_import_preview": _tool_qbot_intervals_comments_import_preview,
    "qbot_intervals_comments_import_execute": _tool_qbot_intervals_comments_import_execute,
    "qbot_cronometer_nutrition_import_preview": _tool_qbot_cronometer_nutrition_import_preview,
    "qbot_cronometer_nutrition_import_execute": _tool_qbot_cronometer_nutrition_import_execute,
    "qbot_wellness_day_get": _tool_qbot_wellness_day_get,
    "qbot_sleep_day_get": _tool_qbot_sleep_day_get,
    "qbot_nutrition_day_get": _tool_qbot_nutrition_day_get,
