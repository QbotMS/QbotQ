...
def _build_report_data(conn, route_id, date_str, start_time, long_stops=0, long_stop_min=0):
    ...
    _month_xss = None
    try:
        _month_xss = int(str(date_str)[5:7]) if date_str else None
    except Exception:
        _month_xss = None
    _xss = _route_physics_xss(conn, route_id, _ftp_mq, _wprime_mq_kj, mass, "normalny", _month_xss)
    if _xss is None:  # fallback: stary estymator IF gdy brak kanonicznej siatki 50 m
...
from qbot3.routes.route_xss_phys import (_wind_clim_for_route, _estimate_route_xss_phys,
                                         _build_route_sig, _route_physics_xss)  # noqa: E402  Tor1: fizyka XSS -> modul wspolny
