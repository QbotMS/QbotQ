...
class TestRouteLandcoverStore(unittest.TestCase):
    def test_route_55798129_landcover_layer_idempotent(self) -> None:
        conn = _db_conn()
        try:
        ...
        first = ensure_route_landcover(route_base_id=route_base_id)
        second = ensure_route_landcover(route_base_id=route_base_id)

        self.assertEqual(first["status"], "OK")
        self.assertEqual(second["status"], "OK")
        self.assertEqual(first["route_base_id"], 1)
        self.assertEqual(first["route_artifact_id"], 306)
        self.assertGreater(first["landcover_layer_count"], 0)
        self.assertEqual(first["landcover_layer_count"], second["landcover_layer_count"])
        ...
        try:
            ...
            self.assertEqual(int(layer_count), first["landcover_layer_count"])
