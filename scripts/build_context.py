...
def build_document(signals: LiveSignals, timestamp: str) -> str:
    ...
    return "\n".join(
        [
            ...
            "- TRENER (osobna strona /trener.html w menu, ikona hantla; do 2026-09-23 zakladka Formy): planer treningow. Backend: qbot_trener_api.py (/api/trener/*, montowany w qbot_web.py przed app.mount), qbot_trener_engine.py (silnik planu tygodnia: Sezon, Mix, okna/zajetosci, Kalendarz, pogoda Open-Meteo, gotowosc), qbot_trener_stats.py (bilans logi/waga, statusy celow, czas, pogoda auto w tle), qbot_trener_workouts.py (sila obwodowa z rotujacym akcentem, wioslarz), qbot_trener_notify.py (Telegram, cron root */15 tick). Tabele qbot_v2.trainer_* (sql/trainer_v1..v4.sql). Front /opt/qbot/web/public/trener.js + trener.css (POZA repo). Albert: narzedzie trainer_week (odczyt) + intent trainer_week w qbot_query_handler przed training_recent. Garmin usuniety. Pelna dok.: docs/TRENER.md.\",\n            "## Dane (fakt, nie szukac na nowo)",
