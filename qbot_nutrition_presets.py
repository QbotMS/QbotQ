...
SAMPLE_TARGET = 30
# Pasmo (kcal) do liczenia makr dla dowolnej wartosci (ryczalt z eventu)
BAND_ANY_KCAL = 250
MIN_KCAL = 1200
...
def _clean_samples(conn):
    """Realne, czyste dni (bez presetow/estimated) -- baza do median makr."""
    rows = conn.execute(
    ...
    for r in rows:
        ...
        if r["tainted"] or kcal < MIN_KCAL:
            continue
        out.append({"kcal": kcal, "carbs": float(r["carbs"] or 0),
                    "prot": float(r["prot"] or 0), "fat": float(r["fat"] or 0)})
        if len(out) >= SAMPLE_TARGET:
            break
    return out


def macros_for_kcal(conn, kcal, band=BAND_ANY_KCAL):
    """Makra dla DOWOLNEJ wartosci kcal (ryczalt z eventu kalendarza).

    Ta sama zasada co presety: mediana makr z realnych, czystych dni o
    spozyciu w pasmie +-band wokol podanej wartosci; gdy brak probek --
    podzial FALLBACK_SPLIT. Zwraca kcal + makra + n_days + low_confidence.
    """
    kcal = int(round(float(kcal)))
    g = [s for s in _clean_samples(conn) if abs(s["kcal"] - kcal) <= band]
    if g:
        macros = {"carbs_g": _med([s["carbs"] for s in g]),
                  "protein_g": _med([s["prot"] for s in g]),
                  "fat_g": _med([s["fat"] for s in g])}
    else:
        macros = _macros_from_split(kcal)
    return {"kcal": kcal, "n_days": len(g), "low_confidence": len(g) < 5, **macros}
